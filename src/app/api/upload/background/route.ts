...
export async function POST(req: NextRequest) {
  ...
  // Upload to Cloudinary — one background per user (overwrite = true)
  const result = await new Promise<{ secure_url: string }>((resolve, reject) => {
    cloudinary.uploader.upload_stream(
      {
        folder:    "violink/backgrounds",
        public_id: `bg_${session.userId}`,
        overwrite: true,
        quality:   "auto",
        fetch_format: "auto",
      },
  ...
  await prisma.profile.update({
    ...
    data:  { bgType: "image", bgValue: result.secure_url },
  });
